...
class LottieController {
  int id;
  MethodChannel _channelOrNull;
  EventChannel _playFinished;
  EventChannel _initialized;
  final VoidCallback onPlayFinished;
  final InitializedCallback onInitialized;
  final List<StreamSubscription> _subscriptions = [];

  LottieController({this.onPlayFinished, this.onInitialized});

  MethodChannel get _channel {
    if (_channelOrNull == null) throw Exception("Lottie controller not properly initialized.");
    return _channelOrNull;
  }
  ...
  initialize(int id) {
    ...
    this._channelOrNull = MethodChannel('${lottiePluginPath}_$id');
    ...
    if (onPlayFinished != null) {
      _subscriptions.add(_playFinished
          .receiveBroadcastStream()
          .where((finished) => finished == true)
          .listen((finished) => onPlayFinished()));
    }

    _subscriptions.add(_initialized.receiveBroadcastStream().listen((_) {
...
typedef InitializedCallback = void Function(LottieController);
